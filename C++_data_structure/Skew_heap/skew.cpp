...
void skew::deletion(node* subtree)
{
  if(subtree!=nullptr)
  {
...
/*while inserting a new element, we perform merge(root,newnode),
  ...
  as a skew heap which contains only one element x*/
void skew::insertion(int x,node* subtree)
{
...
/*while deleting the min element of heap, we just remove the top
  of the skew heap, the do merge for its left child and right
  child*/
void skew::deletemin()
{
  node* old = m_root;
...
/*The merge function takes two skew heaps,root1 and root2,
  and returns one skew heap which contains the union of elements
  from root1 and root2*/
node* skew::merge(node* root1,node* root2)
{
  ...
  else
  {
    ...
    /*swap left child with right child after every merge operation*/
    swapchild(root1);
    return root1;
  }
}
...
void skew::swapchild(node* subtree)
{
  if(subtree!=nullptr)
  {
...
void skew::printorder(int x)
{
  print(m_root,x);
}

int skew::height(node* root)
{
  if(m_root==nullptr)
  {
    return 0;
...
void skew::printlevel(int x,node* root)
{
  if(root == nullptr)
  {
    return;
...
void skew::print(node* root, int x)
{
  if(root!=nullptr)
  {
    if(x==1)
